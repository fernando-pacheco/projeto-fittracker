...
export function ProgramBoard() {
  ...
  async function addSection(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!formSection.title || !formSection.workout) {
        console.error('Título ou ID do treino não fornecidos.')
        return
      }
    ...
    return (
      <ModalRegister
        ...
        onOpenChange={(isOpen) => {
          ...
          if (isOpen) {
            setSectionTitle('') // Limpa o título da seção quando o modal é aberto
          }
        }}
      >
        <div className="grid grid-cols-6 items-center gap-4">
          ...
          <Input
            ...
            value={sectionTitle}
            onChange={e => setSectionTitle(e.target.value)} // Atualiza o estado quando o valor muda
          />
        </div>
      </ModalRegister>
    )
